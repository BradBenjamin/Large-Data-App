...
# READ API_URL FROM ENVIRONMENT VARIABLE (for Docker compatibility)
...
# DEBUG: Show API URL being used (remove in production)
    ...
    if st.button("🔮 Predict User Reaction", use_container_width=True):
        ...
        with st.spinner("Analyzing..."):
            try:
                ...
                st.error("❌ API Request Timeout. Backend may be slow or unresponsive.")
        ...
        if st.button("🚀 Run Batch Prediction", use_container_width=True):
            with st.spinner("Processing batch via API..."):
                try:
                    ...
                    response = requests.post(f"{API_URL}/batch-predict", files=files, timeout=30)
                    response.raise_for_status()
                    ...
                    st.error("❌ Batch processing timeout. Try a smaller file.")
                except requests.exceptions.ConnectionError:
                    st.error(f"❌ Cannot connect to API at `{API_URL}`. Is the backend running?")
                except requests.exceptions.RequestException as e:
                    st.error(f"❌ API Error: {e}")
